...
// Node 41: aligned with b4.x (for waist line adjustment)
// Using same formula as b4_x: shoulderMid_x + (bottomTuckWidth / 2) * scale
$frontNodes['a41'] = [
    'x' => $shoulderMid_x + (($bottomTuckWidth / 2) * $scale),
    'y' => $originY + (($flength + 0.5) * $scale),
    'label' => 'a41',
...
// r6: a7 + 0.25" right, 0.25" up (Point C shifted)
$frontNodes['r6'] = [
...
// r_a9: a9 + 0.25" right, 0.25" up (Point B1 shifted)
$frontNodes['r_a9'] = [
...
// r_a91: a91 + 0.25" right, 0.25" up (Point AB2 shifted)
$frontNodes['r_a91'] = [
...
// r7: a10 + 0.25" right, 0.25" up (Point A shifted)
$frontNodes['r7'] = [
...
// r8: Offset perpendicular to shoulder line (a11→a10), 0.5" away from a11, minus 0.25" along shoulder toward neck
// Uses same perpendicular and along directions calculated for r7
$frontNodes['r8'] = [
    'x' => frontNode('a11','x') + ($perp_x * $r_offset) - ($along_x * $extend_offset),
    'y' => frontNode('a11','y') + ($perp_y * $r_offset) - ($along_y * $extend_offset),
    'label' => 'r8',
    'color' => 'red',
    'code' => '$r8 = a11 + 0.5" perp - 0.25" along shoulder'
];
...
// Armhole cutting line control points - exact copy shifted 0.25" right and 0.25" up
// These use the armhole control points from deepNeck.php ($ctrl1, $ctrl2a, $ctrl2b, $ctrl3)
...
<svg id="frontPatternSvg" width="100%" height="100%"
    ...
    <?php if ($isDevMode): ?>
    <!-- CUTTING LINE (RED) - Dev Mode Only (r5 to r9 segment - armhole/shoulder/neck area) -->
    <!-- Armhole cutting line: exact copy of armhole curve (A→AB2→B1→C) shifted 0.25" right and 0.25" up -->
