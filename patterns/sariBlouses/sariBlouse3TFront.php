...
$minTuckWidth = 2.5;
$tuckWidth = max($bottomTuckWidth, $minTuckWidth);
$tuckDeficit = $tuckWidth - $bottomTuckWidth;  // extra width added (0 if no adjustment)
...
// Node 5: Waist point — shift outward by tuckDeficit to compensate for wider tuck
$frontNodes['a5'] = [
    'x' => $originX + (($qBust + $tuckDeficit) * $scale),
    'y' => $frontNodes['a4']['y'] - (1.0 * $scale),
    'label' => 'Waist',
    'code' => '$a5 = originX + qBust(' . number_format($qBust, 2) . '") + ' . number_format($tuckDeficit, 2) . '", a4.y - 1"'
];
...
// pointA_y is conditional in calculateLShapePoints: originY if bnDepth > 8", else originY + 0.25"
$a10_yOffset = round(($pointA_y - $originY) / $scale, 2);
$frontNodes['a10'] = [
    'x' => $pointA_x,
    'y' => $pointA_y,
    'label' => 'Shoulder',
    'code' => '$a10 = z0.x + shoulder/2, z0.y + ' . number_format($a10_yOffset, 2) . '"'
];
...
$frontNodes['r7'] = [
    ...
    'y' => frontNode('a10','y') - (0.5 * $scale),
    'label' => 'r7',
    'color' => 'red',
    'code' => '$r7 = a10.x + 0.25", a10.y - 0.5"'
];

// r8: offset from a11
$frontNodes['r8'] = [
    'x' => frontNode('a11','x') - (0.25 * $scale),
    'y' => frontNode('a11','y') - (0.5 * $scale),
    'label' => 'r8',
    'color' => 'red',
    'code' => '$r8 = a11.x - 0.25", a11.y - 0.5"'
];
...
$pattiHeight = $blength - $flength + 1;  // +1" ease

// Patti origin - positioned below the front pattern with no gap
...
// p3 = top-right (adjusted: base position minus bust-waist difference + 1")
// qBust - qWaist gives the actual quarter bust-waist difference (ease cancels out)
$pattiNodes['p3'] = [
    'x' => frontNode('a51', 'x'),
    'y' => $pattiOriginY + $p_a5_y_offset,
    'label' => 'p3',
    'code' => '$p3 = a51.x, pattiOriginY + a5_y_offset'
];

// p21 = midpoint on top line between p2 and p3 (for top line adjustment)
$pattiNodes['p21'] = [
    'x' => ($pattiNodes['p2']['x'] + $pattiNodes['p3']['x']) / 2,
    'y' => ($pattiNodes['p2']['y'] + $pattiNodes['p3']['y']) / 2,
    'label' => 'p21',
    'code' => '$p21 = midpoint(p2, p3)'
];
...
// =============================================================================
$frontSvgWidthInches = ($qBust + $tuckDeficit + 2.5);  // 0.5" left margin + qBust + tuckDeficit + 1.5" ease box + 0.5" right margin
...
<svg id="frontPatternSvg" width="100%" height="100%"
    ...
    <!-- Side box hatch fill -->
    <polygon points="<?php echo frontNode('a7','x'); ?>,<?php echo frontNode('a7','y'); ?> <?php echo frontNode('a71','x'); ?>,<?php echo frontNode('a71','y'); ?> <?php echo frontNode('a51','x'); ?>,<?php echo frontNode('a51','y'); ?> <?php echo frontNode('a5','x'); ?>,<?php echo frontNode('a5','y'); ?>"
    ...
    <!-- Ease text (centered in side box between a7-a71 and a5-a51) -->
    <?php
    // Center of the four corners: a7, a71, a5, a51
    $easeTextX = (frontNode('a7', 'x') + frontNode('a71', 'x') + frontNode('a5', 'x') + frontNode('a51', 'x')) / 4;
    $easeTextY = (frontNode('a7', 'y') + frontNode('a71', 'y') + frontNode('a5', 'y') + frontNode('a51', 'y')) / 4;
    // Angle follows a7→a5 direction
    $easeAngle = rad2deg(atan2(frontNode('a5', 'y') - frontNode('a7', 'y'), frontNode('a5', 'x') - frontNode('a7', 'x')));
    ?>
    <text x="<?php echo $easeTextX; ?>" y="<?php echo $easeTextY; ?>"
          font-size="10" fill="#666" text-anchor="middle"
          transform="rotate(<?php echo $easeAngle; ?>, <?php echo $easeTextX; ?>, <?php echo $easeTextY; ?>)">---- ease -----</text>

    <?php if ($isDevMode): ?>
    <!-- CUTTING LINE (RED) - armhole is exact copy of A→AB2→B1→C shifted 0.25" right and 0.25" up -->
    <path d="M <?php echo frontNode('r5','x'); ?>,<?php echo frontNode('r5','y'); ?>
    ...
    <?php if ($isPrintMode): ?>
    <!-- CUTTING LINE (RED) - Print Mode Only (r8 to r7 segment only) -->
    <line x1="<?php echo frontNode('r8','x'); ?>" y1="<?php echo frontNode('r8','y'); ?>"
          x2="<?php echo frontNode('r7','x'); ?>" y2="<?php echo frontNode('r7','y'); ?>"
    ...
    <!-- b1 dot (apex/dart point) - visible in Dev and Print modes -->
    <circle cx="<?php echo frontNode('b1', 'x'); ?>" cy="<?php echo frontNode('b1', 'y'); ?>" r="2"
            fill="#10B981" stroke="#fff" stroke-width="0.5"/>
