<div>
    <div style="float: right;">
        ...
        <a href="../savi/saviPattiDownload.php" style="color:#ffffff"> Download Blouse Patti Design</a>
        </button>
    </div>
...
/* blouse black border */
        ...
        $fnDepth = $_SESSION["fndepth"];
    
        $bPatti1 = "M". ($fLeft + (0.5 * $cIn)) . "," . ($topMargin * $cIn);
        $bPatti2 = "L". ($fLeft + (0.5 * $cIn)) ."," . ((($blength - $flength) + 1.5) * $cIn);
        $bPatti3 = "Q". ((($blength - $flength) + 0.5)* $cIn) ."," . ((($blength - $flength) + 0.5)* $cIn) . "," . (($chest / 8) * $cIn) ."," . ((($blength - $flength) + 0.5)* $cIn);
        $bPatti4 = "L". (($chest / 4) * $cIn) ."," . ((($blength - $flength) + 1.0) * $cIn);
        $bPatti5 = "L". (($chest / 4) * $cIn) . "," . ($topMargin * $cIn);

        $saviBlousePatti = $bPatti1 . $bPatti2 . $bPatti3 . $bPatti4 . $bPatti5 . $bPatti6 . "Z" ;
        $_SESSION["saviPattiBlack"] = $saviBlousePatti;     
    
/* blouse red border */    
    
        $bPattiRed1 = "M". $fLeft . "," . (0.2 * $cIn);
        $bPattiRed2 = "L". $fLeft ."," . ((($blength - $flength) + 2) * $cIn);
        $bPattiRed3 = "Q". ((($blength - $flength) + 1)* $cIn) ."," . ((($blength - $flength) + 0.8)* $cIn) . "," . ((($chest / 8) +1) * $cIn) ."," . ((($blength - $flength) + 1.0)* $cIn);
    
        $bPattiRed4 = "L". ((($chest / 4) +0.5) * $cIn) ."," . ((($blength - $flength) + 1.5) * $cIn);
        $bPattiRed5 = "L". ((($chest / 4) +0.5) * $cIn) . "," . (0.2 * $cIn);
...
// hook patti
        ...
        $hPoint3 = "L" . (($blength - $fnDepth) * $cIn) . "," . ($pattiTop + (3 * $cIn));  
