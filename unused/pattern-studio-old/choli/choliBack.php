<div>
    <div style="float: right;">
        <button class="btn btn-info" >
            <a href="../choli/choliBackDownload.php" style="color:#ffffff">Download Choli Back Design</a>
        </button>
    </div>
...
/* --- blouse back labels --*/
		...
		$bbPoint4 = "L" . 0 .",". (($topMargin + $bnDepth) * $cIn);
		$bbPoint5 = "L" . 0 .",". ((($topMargin + $blength) +0.5) * $cIn);
		...
		$choliBackBlack = $bbPoint1 . $bbPoint2 . $bbPoint3 . $bbPoint4 . $bbPoint5 . $bbPoint6 . $bbPoint7 . $bbPoint8 . $bbPoint9 . $bbPoint10 . "Z";
    
        $_SESSION["choliBackBlack"] = $choliBackBlack;

// -------------- Black line design ----------//

		$sb_point1 = "M" . ((((( $fshoulder / 2) - $shoulder) + $mLeft) * $cIn)) .",". (0.3 * $cIn); 
		$sb_point2 = "L" . ((((( $fshoulder / 2) - $shoulder) + $mLeft) * $cIn)) .",". ($topMargin + ($bnDepth / 2) * $cIn);
		$sb_point3 = "Q" . (((($fshoulder/2) - $shoulder) + $mLeft) * $cIn ). "," .($bnDepth * $cIn).",". 0 .",".($bnDepth * $cIn);
        $sb_point4 = "L" . 0 .",".($bnDepth * $cIn);
		$sb_point5 = "M" . 0 .",". ((($topMargin + $blength) + 0.5) * $cIn);
		$sb_point6 = "L" . ((($waist / 4) + $backDart) * $cIn) .",". (((($topMargin + $blength) + 0.5) ) * $cIn);
		$sb_point7 = "L" . (($chest / 4)* $cIn).",". $chestVertical; // Seam Allowence

// arm hole Curve adjustment calculation
/*
        if (($chest >='30') && ($chest <= '32')) {
             $curveCalc = 1.0;
        } elseif (($chest >='33')&&($chest <= '35')) {
             $curveCalc = 0.75;
        } elseif (($chest >='36')&&($chest <= '38')) {
             $curveCalc = 0.5;
        } else {
             $curveCalc = 1.0;
        }
*/            
		...
		$sb_point8 = "Q" . ((($fshoulder /2) *$cIn) +$mLeft) . "," . ($chestVertical- (0.4 * $cIn)) .",". ((($fshoulder /2) *$cIn)+ $mLeft) .",". ($chestVertical /2);
        ...
        $choliBackGreen = $sb_point1 . $sb_point2 . $sb_point3 . $sb_point5 . $sb_point6 . $sb_point7 . $sb_point7a . $sb_point8 . $sb_point9 . $sb_point10 . $sb_point11;
    
        $_SESSION["choliBackGreen"] = $choliBackGreen;
...
// -------------- brown line - extra Bust design ----------//
        ...
        $choliBackBrown = $brownPoint6 . $brownPoint7;
    
        $_SESSION["choliBackBrown"] = $choliBackBrown;
...
// -------------- Red line Design ----------- //
        ...
        $point4 = "L" . 0 .",".($bnDepth * $cIn);

        $point5 = "M" . 0 .",". ((($topMargin + $blength) + 1) * $cIn);
	...
	$choliBackRed = $point1 . $point2 . $point3 . $point4 . $point5 . $point6 . $point7 . $point8 .  $point10 . $point11;
    
    $_SESSION["choliBackRed"] = $choliBackRed;
...
// fleet Specifications
  ...
  //      $bhApex = ((($fshoulder / 2) - ($shoulder /2)) * $cIn); //((($fshoulder / 2) - ($shoulder /2)) * $cIn);
		...
		$choliBackTucks = "M". $blengthTextLeft .",". ($blengthText + (0.5 * $cIn)) ."L". $chestText .",". (($apex + 1) * $cIn) ."L". $blengthTextRight .",". ($blengthText + (0.5 * $cIn));
    
        $_SESSION["choliBackTucks"] = $choliBackTucks;  
?>
	
<svg width="500" height="550" viewbox = "-50, -20, 500, 550 ">
	<g>
		<path fill="none" stroke="#d3d3d3" stroke-width="0.5" stroke-miterlimit="10" d="<?php echo $choliBackBlack;?>" />
		
		<path fill="none" stroke="#000000" stroke-width="1" stroke-miterlimit="10" d="<?php echo $choliBackGreen;?>" />
		
		<path fill="none" stroke="#000000" stroke-width="0.5" stroke-dasharray="10, 5" stroke-miterlimit="10" d="<?php echo $choliBackBrown;?>" />
				
		<text x="<?php echo $bhApex;?>" y="<?php echo $bvApex;?>"><?php echo $choliBackTuckText;?></text>
		<text x="<?php echo $bhApex;?>" y="<?php echo $bvApex;?>" transform="rotate(-90, 10, <?php echo $bvApex;?>)">'< ---- Fold --- >' </text>
		<path fill="none" stroke="#000000" stroke-width="0.5" stroke-miterlimit="10" d="<?php echo $_SESSION["choliBackTucks"] ?>" /> 
		<path fill="none" stroke="#ff0000" stroke-dasharray="5, 5" stroke-width="0.5" stroke-miterlimit="10"  d="<?php echo $choliBackRed;?>" />
