...
// IMPORTANT: deepNeckCV_ai.php loads measurements from database and stores in $_SESSION['measurements']
if (!function_exists('findDeepNeckControlPoint')) {
    // Try relative path first, then from root
    $aiPath = __DIR__ . '/../../includes/deepNeckCV_ai.php';
    if (!file_exists($aiPath)) {
        $aiPath = $_SERVER['DOCUMENT_ROOT'] . '/includes/deepNeckCV_ai.php';
    }
    require_once $aiPath;
}
...
// Check if measurements are loaded in session by deepNeckCV_ai.php
if (!isset($_SESSION['measurements'])) {
    die("Error: Measurements not loaded. Please ensure deepNeckCV_ai.php has loaded the data.");
}

// Load all measurements from session (already loaded by deepNeckCV_ai.php)
...
$pattiNodes['p3'] = [
    'x' => $pattiOriginX + ($pattiLength * $scale),
    'y' => $pattiOriginY + ($pattiWidth * $scale),
    'label' => 'p3',
    'code' => '$p3 = p2.x, p2.y + pattiWidth'
];
            ...
            <div class="pattern-cell cell-1x1">
                ...
                <div class="svg-container">
                    ...
                    <svg id="patternSvg" width="<?php echo $svgWidth; ?>" height="<?php echo $svgHeight; ?>"
                        ...
                        <!-- ============================================= -->
                        <!-- 2" x 2" MEASUREMENT SCALE (Both Modes) -->
                        <!-- ============================================= -->
                        <!-- Scale box variables calculated in Section 2 -->

                        <g id="measurementScale">
                            <!-- Scale border (2" x 2" square) -->
                            <rect x="<?php echo $scaleX; ?>"
                                  y="<?php echo $scaleY; ?>"
                                  width="<?php echo $scaleSize; ?>"
                                  height="<?php echo $scaleSize; ?>"
                                  fill="none"
                                  stroke="#000"
                                  stroke-width="0.5"/>

                            <!-- Dotted gridlines at 0.5", 1", 1.5" (light gray) -->
                            <!-- Horizontal lines -->
                            <?php foreach ([1, 2, 3] as $i): ?>
                            <line x1="<?php echo $scaleX; ?>"
                                  y1="<?php echo $scaleY + ($i * $scaleIncrement); ?>"
                                  x2="<?php echo $scaleX + $scaleSize; ?>"
                                  y2="<?php echo $scaleY + ($i * $scaleIncrement); ?>"
                                  stroke="#CCCCCC"
                                  stroke-width="0.5"
                            ...
                            <!-- Vertical lines -->
                            <?php foreach ([1, 2, 3] as $i): ?>
                            <line x1="<?php echo $scaleX + ($i * $scaleIncrement); ?>"
                                  y1="<?php echo $scaleY; ?>"
                                  x2="<?php echo $scaleX + ($i * $scaleIncrement); ?>"
                                  y2="<?php echo $scaleY + $scaleSize; ?>"
                                  stroke="#CCCCCC"
                                  stroke-width="0.5"
                                  stroke-dasharray="2,2"/>
                            <?php endforeach; ?>

                            <!-- Tick marks on bottom edge (ruler style) -->
                            <?php for ($i = 0; $i <= 4; $i++): ?>
                                <?php
                                // Every 1" (i=0,2,4) gets a longer tick (0.15"), half-inch marks get medium (0.1")
                                $tickLength = ($i % 2 == 0) ? 0.15 : 0.1;
                                ?>
                                <line x1="<?php echo $scaleX + ($i * $scaleIncrement); ?>"
                                      y1="<?php echo $scaleY + $scaleSize; ?>"
                                      x2="<?php echo $scaleX + ($i * $scaleIncrement); ?>"
                                      y2="<?php echo $scaleY + $scaleSize + ($tickLength * $scale); ?>"
                                      stroke="#000"
                                      stroke-width="1"/>
                            <?php endfor; ?>

                            <!-- Tick marks on right edge (ruler style) -->
                            <?php for ($i = 0; $i <= 4; $i++): ?>
                                <?php
                                // Every 1" (i=0,2,4) gets a longer tick (0.15"), half-inch marks get medium (0.1")
                                $tickLength = ($i % 2 == 0) ? 0.15 : 0.1;
                                ?>
                                <line x1="<?php echo $scaleX + $scaleSize; ?>"
                                      y1="<?php echo $scaleY + ($i * $scaleIncrement); ?>"
                                      x2="<?php echo $scaleX + $scaleSize + ($tickLength * $scale); ?>"
                                      y2="<?php echo $scaleY + ($i * $scaleIncrement); ?>"
                                      stroke="#000"
                                      stroke-width="1"/>
                            <?php endfor; ?>

                            <!-- Scale labels (0" to 2" with 0.5" increments) -->
                            <!-- Bottom edge labels (rotated -90 degrees, reading top to bottom) -->
                            <?php for ($i = 0; $i <= 4; $i++): ?>
                                <?php
                                $labelValue = number_format($i * 0.5, 1);
                                $labelX = $scaleX + ($i * $scaleIncrement);
                                $labelY = $scaleY + $scaleSize + (0.25 * $scale);
                                // Larger font for 0.0", 1.0", 2.0" (i=0,2,4), smaller for 0.5", 1.5" (i=1,3)
                                $fontSize = ($i % 2 == 0) ? (0.20 * $scale) : (0.18 * $scale);
                                ?>
                                <text x="<?php echo $labelX; ?>"
                                      y="<?php echo $labelY; ?>"
                                      ...
                                      transform="rotate(-90 <?php echo $labelX; ?> <?php echo $labelY; ?>)"><?php echo $labelValue; ?>"</text>
                            <?php endfor; ?>

                            <!-- Right edge labels (vertical) -->
                            <?php for ($i = 0; $i <= 4; $i++): ?>
                                <?php
                                $labelValue = number_format($i * 0.5, 1);
                                // Larger font for 0.0", 1.0", 2.0" (i=0,2,4), smaller for 0.5", 1.5" (i=1,3)
                                $fontSize = ($i % 2 == 0) ? (0.20 * $scale) : (0.18 * $scale);
                                ?>
                                <text x="<?php echo $scaleX + $scaleSize + (0.15 * $scale); ?>"
                                      y="<?php echo $scaleY + ($i * $scaleIncrement); ?>"
                                      font-size="<?php echo $fontSize; ?>"
                                      font-family="Arial, sans-serif"
                                      fill="#000"
                                      text-anchor="start"
                                      dominant-baseline="middle"><?php echo $labelValue; ?>"</text>
                            <?php endfor; ?>

                            <!-- Scale title (3 lines, vertically centered, 0.3" from left border) -->
                            <?php $lineHeight = 0.4 * $scale; // Line height for text spacing ?>
                            <text x="<?php echo $scaleX + (0.3 * $scale); ?>"
                                  y="<?php echo $scaleY + ($scaleSize / 2) - $lineHeight; ?>"
                                  font-size="<?php echo 0.25 * $scale; ?>"
                                  font-family="Arial, sans-serif"
                                  font-weight="bold"
                                  fill="#000"
                                  text-anchor="start">Printer</text>
                            <text x="<?php echo $scaleX + (0.3 * $scale); ?>"
                                  y="<?php echo $scaleY + ($scaleSize / 2); ?>"
                                  font-size="<?php echo 0.25 * $scale; ?>"
                                  font-family="Arial, sans-serif"
                                  font-weight="bold"
                                  fill="#000"
                            ...
                            <text x="<?php echo $scaleX + (0.3 * $scale); ?>"
                                  ...
                                  font-weight="bold"
                                  fill="#000"
                                  text-anchor="start">2"x2" Scale</text>
                        </g>
