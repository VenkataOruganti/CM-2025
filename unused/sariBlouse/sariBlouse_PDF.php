...
} elseif (isset($_GET['measurement_id'])) {
    $measurementId = intval($_GET['measurement_id']);
}

// Try to get pattern from session cache first
if ($measurementId) {
...
// If no session data, load from database and generate pattern
if ($patternData === null && $measurementId) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            SELECT m.*,
                   COALESCE(c.customer_name, m.customer_name, u.username, 'Customer') as display_name
            FROM measurements m
        ...
        if ($row) {
            // Include sariBlouse.php to generate pattern SVGs
            $_GET['customer_id'] = $row['customer_id'] ?? null;
            $_GET['measurement_id'] = $measurementId;

            // Capture sariBlouse.php output (it sets session variables)
            ob_start();
            include __DIR__ . '/sariBlouse.php';
            ob_end_clean();

            // Now try to get pattern data from session
            $cacheKey = "pattern_" . $measurementId;
            if (isset($_SESSION[$cacheKey])) {
                $patternData = $_SESSION[$cacheKey]['data'];
            } elseif (isset($_SESSION['latest_pattern'])) {
                $cacheKey = $_SESSION['latest_pattern'];
                if (isset($_SESSION[$cacheKey])) {
                    $patternData = $_SESSION[$cacheKey]['data'];
                }
        ...
        error_log("sariBlouse_PDF.php - Database error: " . $e->getMessage());
    }
}
...
if ($patternData === null) {
    ...
    die("ERROR: No pattern data found. Please generate a pattern first using sariBlouse.php or provide a valid measurement id.");
}
...
function addTileReference($pdf, $pageWidth, $pageHeight, $tileNumber, $totalTiles, $patternName, $margin) {
    ...
    // Logo bottom: logoY + logoHeight = -0.5 + 2.7 = 2.2"
    // URL is at logoY + logoHeight - 1.08 = 1.12"
    // Place tile info below URL with gap to avoid overlap
...
function addSummarySheet($pdf, $pageWidth, $pageHeight, $margin, $patternData, $tileInfo, $paperSize) {
    ...
    // =========================================================================
    // PATTERN METADATA (left) and PRINTING INSTRUCTIONS (right)
    // =========================================================================
    ...
    $rightColWidth = $contentWidth - $leftColWidth - $colGap; // Remaining width for thumbnails
...
addSummarySheet($pdf, $paper['width'], $paper['height'], $summaryMargin, $patternData, $tileInfo, $paperSize);
